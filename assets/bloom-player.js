...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initPlayer(root, data, mode) {
        ...
        let currentIndex = 0;
        let isPlaying = false;
        let mediaEl = null; // Defined here, accessible to all child functions
        let transcriptData = null;
        let bookmarks = [];

        // --- RESIZE INTELLIGENCE (Compact Mode) ---
        if (mode === 'full') {
        ...
        if (mode === 'mini') {
            ...
            root.innerHTML = `
                        ...
                        <div class="k-bloom-controls">
                            ...
                            <div class="k-buttons">
                                ...
                                <div class="k-actions">
                                    ...
                                    <button class="k-btn-icon" id="k-text-btn" title="Read Transcript" style="opacity:0.3; cursor:default;">📝</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div id="k-read-modal" class="k-read-modal" style="display:none;">
                    <div class="k-modal-head"><h3>Read Along</h3><button id="k-close-modal">✕</button></div>
                    <div id="k-read-content" class="k-read-content"></div>
                </div>
            `;
        }
        ...
        const readModal = root.querySelector('#k-read-modal');
        const readContent = root.querySelector('#k-read-content');
        ...
        function updateProgress() {
            ...
            if (transcriptData && readModal && readModal.style.display !== 'none') syncText(mediaEl.currentTime);
        }
                    ...
                    .then(json => {
                        ...
                        if(json.results) {
                            json.results.forEach(res => {
                                if(res.alternatives && res.alternatives[0].words) {
                                    res.alternatives[0].words.forEach(w => {
                                        transcriptData.push({
                                            word: w.word,
                                            start: parseFloat(w.startOffset.replace('s','')),
                                            end: parseFloat(w.endOffset.replace('s',''))
                                        });
                                    });
                                }
                            });
        ...
        function syncText(time) {
            const words = readContent.querySelectorAll('.k-word');
            words.forEach(w => {
                const start = parseFloat(w.dataset.start);
                const end = parseFloat(w.dataset.end);
                if (time >= start && time <= end) {
                    w.classList.add('active');
                    w.scrollIntoView({behavior: "smooth", block: "center", inline: "nearest"});
                } else {
        ...
        if (mode === 'full') {
            ...
            const closeModal = root.querySelector('#k-close-modal');
            ...
            if(textBtn) textBtn.onclick = () => { if(transcriptData) readModal.style.display = 'flex'; };
            if(closeModal) closeModal.onclick = () => { if(readModal) readModal.style.display = 'none'; };
            ...
            if(tabChapters) tabChapters.onclick = () => {
                tabBookmarks.classList.remove('active'); tabChapters.classList.add('active');
                renderList();
            };
        }
